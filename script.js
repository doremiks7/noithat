		...
		if(cart) {
			...
			jQuery.each(cart.items,function(i,item){
				...
				if(item.image != null)
					...
					tr.find('.item-image').html("<img src='//theme.hstatic.net/200000289701/1000851936/14/no_image.jpg?v=37'>");
...
function clone_item(product,i){
	...
	if ( product.image == null ) {
		item_product.find('img').attr('src','//theme.hstatic.net/200000289701/1000851936/14/no_image.jpg?v=37').attr('alt', product.url);
	} else {
...
jQuery(document).ready(function(){
	...
	if (window.template.indexOf('index') > -1) {
		...
		if ($('.list-slider-banner').length > 0) {
			...
			var Num_timeAutoPlay = parseInt(timeAutoPlay);
		...
		if ($('.wrapper-home-banner .owl-carousel').length > 0) {		
			var checkBanner =	$('.wrapper-home-banner .home-banner-pd').length;			
